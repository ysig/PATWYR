...
def preprocess(img, input_size, random_pad=False):
    ...
    if random_pad:
        t = random.randint(0, wt - new_size[0])
        target[0:new_size[1], t:t+new_size[0]] = img
    else:
...
def load_batch_image(max_img=2, transform=False):
    # Each batch should have 
    return [preprocess(os.path.join('debug-data', f"{i}.png"), (2227, 64), random_pad=transform) for i in range(1, max_img+1)]

if __name__ == "__main__":
        # load two images
    bi = load_batch_image()
    for i in range(len(bi)):
        cv2.imwrite( f'preprocess_{i}.png', bi[i])
    for i in range(len(bi)):
        x = augmentation(bi[i], rotation_range=1.5, scale_range=0.5, height_shift_range=0, width_shift_range=0, dilate_range=5, erode_range=7)
        print(x.shape)
        cv2.imwrite( f'preprocess_a_{i}.png', x)
    bi = load_batch_image(transform=True)
    for i in range(len(bi)):
        cv2.imwrite( f'preprocess_p{i}.png', bi[i])
